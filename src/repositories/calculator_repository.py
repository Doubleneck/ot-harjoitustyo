class CalculatorRepository:
    ...
    def stats(self):
        ...
        if resultall != 0:
            resultadd=cursor.execute("select count(*) from operations "
                                     " where operation='+'").fetchone()[0]
            resultsub=cursor.execute("select count(*) from operations "
                                     "where operation='-'").fetchone()[0]
            resultdiv=cursor.execute("select count(*) from operations "
                                     "where operation='/'").fetchone()[0]
            resultmul=cursor.execute("select count(*) from operations "
                                    "where operation='*'").fetchone()[0]
            resultsqrt=cursor.execute("select count(*) from operations where "
                                      "operation='sqrt'").fetchone()[0]
            resultexp=cursor.execute("select count(*) from operations "
